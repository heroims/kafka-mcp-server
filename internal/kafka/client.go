...
func (c *Client) ConsumeMessagesByTime(ctx context.Context, topics []string, startTime, endTime int64, maxMessages int, reset bool) ([]Message, error) {
	...
	tempClient, err := kgo.NewClient(kgo.SeedBrokers(c.cfg.KafkaBrokers...))
	if err != nil {
		return nil, fmt.Errorf("failed to create temporary Kafka client: %w", err)
	}
	adminClient := kadm.NewClient(tempClient)
	var listed kadm.ListedOffsets

	if reset {
		listed, err = adminClient.ListOffsetsAfterMilli(
			context.Background(),
			startTime,
			topics...,
		)
	} else {
		listed, err = adminClient.ListCommittedOffsets(
			context.Background(),
			topics...,
		)
	}
	...
	if reset {
		_, err = adminClient.DeleteGroup(context.Background(), c.cfg.KafkaClientID)
		if err != nil {
			slog.WarnContext(ctx, "Failed to delete consumer group", "group", c.cfg.KafkaClientID, "err", err)
		}
	}
	_, err = adminClient.CommitOffsets(context.Background(), c.cfg.KafkaClientID, listed.Offsets())
	if err != nil {
		slog.WarnContext(ctx, "Failed to commit offsets", "group", c.cfg.KafkaClientID, "err", err)
	}
	tempClient.Close()
	opts := []kgo.Opt{
